...
# Router to connect private subnet to sharednet1
...
# -------------------------------------------------------------------
# Ports on sharednet1 (for floating IP assignment) — one per node
# -------------------------------------------------------------------
...
# Ports on private network — one per node
resource "openstack_networking_port_v2" "private_ports" {
  ...
  fixed_ip {
    subnet_id  = openstack_networking_subnet_v2.private_subnet.id
    ip_address = "192.168.1.1${index(tolist(toset(["node1", "node2", "node3"])), each.key) + 1}"
  }
}

# -------------------------------------------------------------------
# VM instances — 3-node cluster (node1 = control plane + worker,
#                                 node2/node3 = workers)
# -------------------------------------------------------------------
resource "openstack_compute_instance_v2" "nodes" {
  ...
  # sharednet1 first so Chameleon assigns it a routable address
  network {
    port = openstack_networking_port_v2.sharednet1_ports[each.key].id
  }

  network {
    port = openstack_networking_port_v2.private_ports[each.key].id
  }
...
# -------------------------------------------------------------------
# Floating IP — only node1 needs a public IP (SSH jump host +
#               external access to Immich / MLflow / Argo)
# -------------------------------------------------------------------
...
# -------------------------------------------------------------------
# Persistent volume for MLflow artifacts (attached to node1)
# -------------------------------------------------------------------
resource "openstack_blockstorage_volume_v3" "mlflow_artifacts" {
  name = "mlflow-artifacts-immich-${var.suffix}"
  size = 50  # GB — adjust as needed
}

resource "openstack_compute_volume_attach_v2" "mlflow_artifacts_attach" {
  instance_id = openstack_compute_instance_v2.nodes["node1"].id
  volume_id   = openstack_blockstorage_volume_v3.mlflow_artifacts.id
}
